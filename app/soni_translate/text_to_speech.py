...
def sanitize_file_name(file_name):
    ...
    # Normalize the string to NFKD form to separate combined characters and diacritics
...
def load_piper_model(
    ...
    if not model_path.exists():
        # Load voice info
        voices_info = get_voices(data_dir, update_voices=update_voices)
...
def audio_segmentation_to_voice(
    ...
    pattern_gradio = re.compile(r".* GRADIO$")
    ...
    speakers_gradio = find_spkr(pattern_gradio, speaker_to_voice, all_segments)
    ...
    filtered_gradio = filter_by_speaker(speakers_gradio, all_segments)
    ...
    if filtered_gradio["segments"]:
        logger.info(f"GRADIO TTS: {speakers_gradio}")
        segments_gradio_tts(filtered_gradio, TRANSLATE_AUDIO_TO)
...
def segments_gradio_tts(filtered_gradio_segments, TRANSLATE_AUDIO_TO):
    """
    Process text segments using Gradio TTS via direct HTTP requests
    """
    import requests
    import json
    from .language_configuration import GRADIO_VOICES_LIST
    
    base_url = "http://127.0.0.1:7870/gradio_api/call"
    sampling_rate = 24000

    def parse_sse_response(response_text):
        """Parse SSE response to get file path"""
    ...
    for segment in tqdm(filtered_gradio_segments["segments"]):
        speaker = segment["speaker"]
        text = segment["text"].strip()
        start = segment["start"] 
        tts_name = segment["tts_name"]
        
        # Визначаємо endpoint в залежності від обраного голосу
        endpoint = "/synthesize_single" if "Single Speaker (UA)" in tts_name else "/synthesize_multi"
        voice_path = GRADIO_VOICES_LIST[tts_name]
        ...
        try:
            ...
            payload = {
                "data": [
                    text,
                    voice_path if endpoint == "/synthesize_multi" else None,
                    1.0
                ]
            }
            # Видаляємо None значення для single speaker
            if endpoint == "/synthesize_single":
                payload["data"] = [d for d in payload["data"] if d is not None]
            ...
            if response.status_code == 200:
                ...
                logger.debug(f"Got event_id: {event_id}")
                
                # Get result
                result = requests.get(f"{base_url}{endpoint}/{event_id}")
                
                if result.status_code == 200:
                    audio_path = parse_sse_response(result.text)
                    ...
                    if audio_path:
                        ...
                        # Save file
                        data_tts = pad_array(data, sampling_rate)
                        write_chunked(
                            file=filename,
                            samplerate=sampling_rate,
                            data=data_tts,
                        ...
                        raise Exception("Could not parse audio path from response")
                else:
                    raise Exception(f"Failed to get result: {result.status_code}")
            else:
                raise Exception(f"Failed to get response: {response.status_code}")

        except Exception as error:
            logger.error(f"Error processing segment: {error}")
            error_handling_in_tts(error, segment, TRANSLATE_AUDIO_TO, filename)
