...
class TTS_Info:
    def __init__(self, piper_enabled, xtts_enabled):
        ...
        self.list_gradio = list(GRADIO_VOICES_LIST.keys())
...
def create_gui(theme, logs_in_gui=False):
    with gr.Blocks(theme=theme) as app:
        ...
        # Новая вкладка "Multi-Translate"
        with gr.Tab("Multi-Translate"):
            with gr.Column():
                with gr.Accordion("Multi-Translate Settings", open=True):
                    with gr.Row():
                        ...
                        voice_file_input = gr.File(
                            label="Upload Voice File (MP3/MP4)",
                            file_types=[".mp3", ".mp4"],
                            interactive=True,
                        )
                        music_file_input = gr.File(
                            label="Upload Music File (MP3/MP4)",
                            file_types=[".mp3", ".mp4"],
                            interactive=True,
                        )
                    
                    with gr.Row():
                        start_button = gr.Button("Start Multi-Translate", variant="primary")
                    ...
                    with gr.Row():
                        log_output = gr.Textbox(label="Log Output", lines=10, interactive=False)
                    
                    with gr.Row():
                        download_link = gr.File(label="Download Final Archive", visible=False)

            def start_multi_translate(srt_file, voice_file, music_file, progress=gr.Progress()):
                try:
                    # Создаем папки, если они не существуют
                    os.makedirs("../INPUT-VIDEO", exist_ok=True)
                    os.makedirs("../MUSIC", exist_ok=True)
                    os.makedirs("../FINAL-ZIP", exist_ok=True)
                    
                    # Очистка папок перед началом работы
                    remove_directory_contents("../INPUT-VIDEO")
                    remove_directory_contents("../MUSIC")
                    remove_directory_contents("../FINAL-ZIP")
                    
                    # Копирование загруженных файлов в соответствующие папки
                    if srt_file:
                        shutil.copy(srt_file.name, "../INPUT-VIDEO/")
                    if voice_file:
                        shutil.copy(voice_file.name, "../INPUT-VIDEO/")
                    if music_file:
                        shutil.copy(music_file.name, "../MUSIC/")
                    
                    # Запуск процесса перевода
                    progress(0, desc="Starting translation process...")
                    
                    # Вызов скрипта translate.py
                    try:
                        logger.info("Calling translate.py...")
                        result = subprocess.run(
                            ["python", "/app/translate.py"],  # Путь к скрипту
                            capture_output=True,
                            text=True,
                            check=True,
                        )
                        # Логируем вывод скрипта
                        logger.info(f"translate.py output: {result.stdout}")
                        if result.stderr:
                            logger.error(f"translate.py errors: {result.stderr}")
                        
                        # Передаем вывод в текстовое поле логов
                        log_output = f"translate.py output:\n{result.stdout}\n"
                        if result.stderr:
                            log_output += f"translate.py errors:\n{result.stderr}\n"
                    except subprocess.CalledProcessError as e:
                        logger.error(f"Error running translate.py: {e}")
                        log_output = f"Error running translate.py: {e}\n"
                        raise Exception("Failed to run translate.py")
                    
                    progress(1.0, desc="Archiving translation...")
                    
                    # Проверяем, создан ли архив
                    archive_path = "../FINAL-ZIP/final.zip"
                    if not os.path.exists(archive_path):
                        logger.error("Archive not found. Check if translate.py created the final files.")
                        log_output += "Archive not found. Check if translate.py created the final files.\n"
                        return gr.update(value=None, visible=False), log_output
                    
                    # Возвращаем ссылку на скачивание архива и логи
                    return gr.update(value=archive_path, visible=True), log_output
                
                except Exception as e:
                    logger.error(f"Error during multi-translate: {e}")
                    log_output = f"Error during multi-translate: {e}\n"
                    return gr.update(value=None, visible=False), log_output

            start_button.click(
                start_multi_translate,
                inputs=[srt_file_input, voice_file_input, music_file_input],
